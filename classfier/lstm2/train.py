...
flags.DEFINE_integer('batch_size',1,'the batch_size of the training procedure')
flags.DEFINE_float('lr',0.1,'the learning rate')
flags.DEFINE_float('lr_decay',0.5,'the learning rate decay')
flags.DEFINE_integer('vocabulary_size',50000,'vocabulary_size')
...
flags.DEFINE_integer('seq_length',36,'max_len of training sentence')
...
flags.DEFINE_integer('num_epoch',0,'num epoch')
...
def sample(dataloader,sess,model):
        for i,(x,mask) in enumerate(dataloader.get_batches('sample')):
            feed = {model.input_data:x,model.targets:np.zeros(len(x),dtype=np.int64),model.mask:mask}
            loss,acc,pre = sess.run([model.cost,model.accuracy,model.pre],feed)
            print pre
