...
flags.DEFINE_integer('filter_list',[4,8,16,32],'LSTM hidden neural size')
flags.DEFINE_integer('filter_num',4,'LSTM hidden layer num')
flags.DEFINE_string('data_path','data','dataset path')
flags.DEFINE_integer('seq_length',40,'max_len of training sentence')
...
flags.DEFINE_integer('num_epoch',25,'num epoch')
flags.DEFINE_integer('check_point_every',1000,'checkpoint every num epoch ')
...
flags.DEFINE_integer('max_decay_epoch',20,'num epoch')


class Config():
    data_dir = FLAGS.data_path
    vali_rate = FLAGS.vali_rate
    filter_list = FLAGS.filter_list
...
def validation(dataloader,sess,model):
        ...
        for i,(x,y,mask) in enumerate(dataloader.get_batches('validation')):
            ...
            #    if y[v]!=pre[v]:
            #        wf.write(sen[index]+'\t'+str(y[v])+'\t'+str(pre[v])+'\n')
            #    index += 1
            end = time.time()
